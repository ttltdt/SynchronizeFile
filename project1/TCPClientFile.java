...
/**
 * A client that will send files and folder to a backup server
 * @author Tung Thai 
 * 
 */
...
public class TCPClientFile {
	...
	private final String serverIPAddress = "192.168.0.22";
	 ...
	 * 
	 * @param path
	 *            serialized file information's path
	 * @throws IOException
	 */
	public void buildDirectory(Path path) throws IOException {
	 ...
	 * @throws IOException
	 */
	public void iniSendFile(Path path) throws IOException {
		input = new ObjectInputStream(Files.newInputStream(path));
	...
	/**
	 * Creating folder's to store old information and new information
	 * 
	 * @param oldLog
	 *            folder to store old information
	 * @param newLog
	 *            folder to store new information
	 */
	 ...
	 * 
	 * @param oldLog
	 *            folder that store old information
	 * @param newLog
	 *            folder that store new information
	 */
		...
		public void run() {
			try {
				...
				if (!Files.isDirectory(Paths.get(file.getAbsPath()))) {
					outStream.writeLong(1);
					outStream.flush();
					threadSocket = new Socket(serverIPAddress, port);
